...
3. 乘方括号错误：必须区分 -2² 和 (-2)² ——前者表示"2²再取负"，结果为 -4；后者表示"先加括号再乘方"，结果为 4。只有当学生的计算结果**实际体现出**把两者弄反了（例如把 -2² 当成 (-2)² 来算、得到了 4 而不是 -4；或者反过来把 (-2)² 当成 -2² 来算、得到了 -4 而不是 4），才判定为该错误。如果学生自始至终都正确保留了 -2² 这种写法、只是暂时还没有把它代入具体数值参与下一步运算（这种"尚未计算"的情况属于第7类"预处理不彻底"，请归入第7类），不要重复判定为本类错误。【重要澄清，避免误判】把"÷(某数)ⁿ"改写成"×(1/某数)ⁿ"是完全合法的等价变形（因为 1÷aⁿ = (1÷a)ⁿ 恒成立，例如 ÷(-4)³ 等价于 ×(-1/4)³，两者数值必然相等，不存在"数值不同"的问题）；同理把除数直接写成分数形式、连同指数一起保留（如把 ÷(-4)³ 写成 ×(-1/4)³）也是合法的，不要因为学生把"底数"从整数换成了分数形式，就误判为"混淆了底数""数值不同"，这类等价变形本身没有错，只有当学生后续没有把这个式子算出正确数值时，才可能触发第7类"预处理不彻底"。
...
判分要求：只要解题过程中的任意一步触发上述七类错误之一，该题 overall 必须判定为"有问题"并相应扣分，即使学生最终写出的数值答案与正确答案一致（蒙对/抵消纠错），也不能给"正确"判定。请完整检查解题过程的每一步，不要在发现第一处错误后就停止分析——但 issues 数组最多列出 2-3 个最值得指出的错误（按出现顺序优先选择前面步骤的错误），避免一次性给出过多批注让学生困惑；若同一类错误在多个步骤重复出现，只需挑一次代表性的指出即可。
