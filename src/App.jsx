...
const PROBLEM_TYPES = {
  ...
  "混合运算":       { steps: ["划分模块","模块内定号/化简","提取公因数(可选)","模块相加减"], color: "#f2795c" },
...
【转写自洽性自查，极其重要】手写数字里"3"和"9"、"1"和"7"等笔画相近，图片模糊或字迹潦草时你转写(transcription)某一行的数字有可能认错。在判定某一步"计算错误"或"漏负号"之前，请先做一次自洽性检查：把你转写的这一行数字，代入到学生下一行实际写出的计算结果里验证——如果按你转写的数字算不出学生下一行写的得数，但是把某个形近数字（如把你转写的9改成3，或反过来）替换一下就能让前后两行完全对得上，这种情况下**应当优先怀疑是你自己转写认错了字**，而不是判定学生计算错误；请回头修正 transcription 里这个数字为更可能的正确读法，再基于修正后的转写重新判断这一步是否真的有问题。只有当无论怎么合理修正转写、前后两行依然对不上时，才能判定为学生本人的计算错误。
...
【极其重要，验证漏负号前必须先做这一步，否则会造成大量误判】当你看到上一行有 N 个项、下一行出现了几个和上一行数字对不上的新数字时，**必须先尝试寻找一种把上一行全部 N 个项完整、无遗漏地划分成若干组、每组内部做加减后能得到下一行这几个数字的分组方式**（每一项必须带着自己原来的符号和数值完整参与某一组，不能只取一项的一部分，也不能凭空引入不存在的数字），不能不假思索地假设学生一定是按书写顺序从左到右两两顺次相加、对不上就直接判"漏负号"。请按下面两种情况分别处理：
(a) 如果存在至少一种合法分组，分组内部的加减本身也算对了，只是学生跳过了"先把要凑整/抵消的项搬到一起排好"这一中间步骤、直接写出了分组后的得数——这不是漏负号，也不是计算错误，应归类为第7类"预处理不彻底（跳步）"，description 必须如实描述为"跳过了移项归类这一步，直接写出分组结果"，禁止编造一套学生实际上没有做过的"从左到右计算、算错了符号"的演算过程来解释，那样的描述内容如果不是学生实际的计算路径，就是凭空捏造，会误导学生以为自己犯了并未犯过的符号错误。
(b) 只有当穷尽所有可能的分组方式后，依然没有任何一种分组能推出学生写的这个数字，才能判定为真正的漏负号或计算错误，且 description 里给出的错误演算必须是针对学生实际使用的那种分组方式来验算的，不能是你自己臆造的一种分组。
...
【混合运算与有理数运算的分类界限，请先判断再选择规则】如果算式只包含数字的加减（哪怕中间某一项本身是乘方算出来的具体数值），本质上是一条加减链条，判定为"有理数运算"类型，按上面的规则检查。如果算式本身包含乘、除、乘方，且必须先把算式划分成若干"模块"（乘除法连在一起算作一个模块，模块与模块之间才是加减关系）才能继续往下做，判定为"混合运算"类型，改用下面这套规则检查，不要套用"有理数运算"里的规则（尤其是"漏负号"，那是加减链条专用的判断，不适用于乘除模块内部的定号判断）。

【混合运算专项标准（初一，含乘方、乘除、括号，需先分模块）】重点同样不是"答案对不对"，而是"每一步处理是否规范"。请按以下框架逐步核查：

第一步，模块划分：确认原式被正确拆成了几个乘除模块（模块内部全是乘除法和乘方运算，模块之间是加减关系）。后续所有判断都建立在"模块"这个单位上。

第二步，逐模块核实定号是否正确：对每一个乘除模块，数一数其中真正参与乘除运算、本身为负的因子个数（偶次幂的结果恒为正，不计入负因子个数）。奇数个负因子，模块结果为负；偶数个，为正。检查学生是否正确走了"数负因子个数→奇负偶正→确定模块符号"这个判断过程，还是直接用绝对值相乘/相除的结果当成了最终答案、跳过了定号这一步。
- 如果某个模块该为负却被算成正（或反之），判定为【定号判断失误】，description 必须具体点出这个模块里有几个负因子、奇偶性、正确应该是什么符号，不能笼统说"符号错误"。
- 【与"漏负号"的区分，务必遵守】"漏负号"专属于加减链条内部某一项自带符号的丢失，"定号判断失误"专属于乘除模块的整体定号判断被跳过——这是两种完全不同的错误机制，绝不能混用，混合运算题目里出现的符号问题，只要发生在乘除模块内部，一律归类为【定号判断失误】。

第三步，核实模块边界有没有被看串：检查是否存在"后一模块的数字或符号被错误并入前一模块"，或者"模块之间的运算符被看错"的情况（比如把加号连接的两个独立模块，误当成其中一个模块内部还存在除法关系）。这类判定为【模块边界混淆】，description 必须点名具体是哪个数字被错误拼接进了哪个模块、正确的模块划分应该是怎样。

第四步，核实预处理是否到位：检查小数是否化成了分数、带分数是否化成了假分数、除法是否转成了乘法、乘方是否已优先算出具体数值。如果这类转换没做，但学生后续的分组/计算逻辑本身是自洽的（能倒推出他后面写的数字），判定为【预处理不彻底（跳步）】，不要判定为计算错误。【重要】纯粹的格式转换（比如把 -0.25 写成 -1/4，数值不变、只是写法不同）不涉及任何运算，不能仅因为这一步"数值的写法变了"就判定为计算错误——这是等价改写，必须先验证是否等价，不等价才是真正的错误。

第五步，如果本题使用了提取公因数/公因式的技巧，额外核查：
- 检查提取公因数后，每一项系数是否根据新公因数重新做了定号判断，而不是直接照抄原式里的运算符号。如果系数符号错误，判定为【提取公因数-系数未重新定号】，description 必须点出具体是哪一项、正确系数应该是多少。
- 检查"公因数×系数之和"这最后一步乘法本身的符号处理。如果前面提取和系数计算都正确，只是这最后一步符号弄丢了，判定为【提取公因数-收尾符号丢失】。
- 如果算式明显可以提取公因数/公因式、学生没有使用这个技巧，但按模块老老实实计算也得出了正确结果：这不算错误，不生成 issue，可以在 praise 字段里提一句优化建议（比如"这道题几个模块有公因数XX，下次可以尝试提取出来简化计算"），不要放进 summary、也不要影响 overall 和 score。

第六步，区分"纯计算失误"与"数字抄写失误"：
- 如果某一步纯粹是数值算错（约分倍数关系算错、乘法口算错），且不涉及任何符号判断，判定为【纯计算失误】。
- 如果学生的方法和符号判断全部正确，但某个数字与前一步对不上，且这种对不上无法用任何计算逻辑或符号规则解释：先检查是否为形近数字混淆（如3与9、1与7），能用这种混淆解释的，倾向于识别问题而非学生错误，description 中明确写"疑似识别有误，无法完全确定是书写问题还是识别问题"，不归咎学生；只有排除了形近字符歧义、且数字写得清晰工整仍然对不上时，才判定为【数字抄写失误】，description 只需提醒检查这一步数字是否抄写一致，不牵扯任何数学规则讲解。

判分要求：与有理数运算部分一致——只要触发上述任意一类错误，overall 必须判定为"有问题"并相应扣分，即使最终数值凑巧正确也不能判"正确"；issues 数组同样最多列出2-3条最值得指出的错误。
          ...
          <div style={{marginBottom:14}}>
            ...
            {showTips&&(
              <div style={{background:"#fdf8ec",border:"1px solid #e8d8b0",borderTop:"none",borderRadius:"0 0 10px 10px",padding:"12px 16px",fontSize:12.5,color:"#6a5a3a",lineHeight:1.9}}>
                ...
                <div style={{marginBottom:6}}>🔢 <b>别跳步，"移项归类"这一步要单独写出来</b>——把要凑整、要抵消的项先搬到一起排好，这一行写完了再往下算得数，不要直接从化简符号跳到最终分组结果，不然 AI 和彭老师都没法确认你是怎么想的</div>
